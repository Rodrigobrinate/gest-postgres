...
Pedido explícito do usuário a partir de um relatório de auditoria (`RELATORIO_SEGURANCA.md`, 33 itens, não gerado nesta sessão — a plataforma já tinha sido revisada antes por outra rodada). "Resolva todos" — 32 dos 33 itens fechados (item 31, rotação/AAD de chave de criptografia, e item 32, `sslmode=disable` interno, seguem como debt reconhecido, o próprio relatório já os classificava como futuro/aceitável). Verificado só via `go build`/`go vet` + `tsc`/`eslint`/`next build` — **nada disso rodou contra o droplet ainda**, e mudanças de RBAC/CORS/cookie merecem teste manual de login/sessão antes de ir pra produção.
