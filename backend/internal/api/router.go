...
func NewRouter(serverService *server.Service, infraService *infra.Service, authService *auth.Service, allowedOrigins []string, trustedProxies []*net.IPNet) http.Handler {
	...
	mux.HandleFunc("GET /api/v1/servers/{id}/tables/{schema}/{table}/columns", detail.TableColumns)
	mux.HandleFunc("GET /api/v1/servers/{id}/tables/{schema}/{table}/rows", detail.TableRows)
	mux.HandleFunc("POST /api/v1/servers/{id}/tables/{schema}/{table}/rows", detail.InsertTableRow)
	mux.HandleFunc("PATCH /api/v1/servers/{id}/tables/{schema}/{table}/rows", detail.UpdateTableRow)
	mux.HandleFunc("DELETE /api/v1/servers/{id}/tables/{schema}/{table}/rows", detail.DeleteTableRow)
